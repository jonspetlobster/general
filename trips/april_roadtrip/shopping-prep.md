# Shopping & Prep List

## 🚴 Mountain Bikes - Megan & Max

### Bike Options (New)

**Budget-Friendly ($500-800):**
- **Polygon Xtrada 5** (~$600) — great value hardtail, 1x drivetrain
- **Vitus Nucleus 27 VR** (~$550) — solid entry-level
- **Giant Talon** (~$700) — reliable brand, good components

**Mid-Range ($800-1500):**
- **Specialized Rockhopper Comp** (~$1,100) — excellent hardtail
- **Trek Marlin 8** (~$1,100) — versatile trail bike
- **Canyon Stoic 4** (~$1,300) — great geometry, solid components

**Full Suspension Entry (~$1500-2500):**
- **Giant Stance 29** (~$2,000) — if you want to go full squish

**Where to Buy:**
- **Mike's Bikes** (Santa Cruz/Bay Area) — local shop, test rides
- **REI** (Capitola) — returns are easy, membership dividend
- **Online:** Canyon, Polygon (direct-to-consumer pricing)

**Size Guide:**
- Measure inseam, use brand size charts
- Test ride if possible (especially for Megan)
- Moab = technical trails, go for sturdy over ultralight

### Bike Accessories (Per Person)

- [ ] Helmet (MIPS recommended)
- [ ] Gloves
- [ ] Padded shorts or bibs
- [ ] Hydration pack (CamelBak, Osprey) — 2-3L capacity
- [ ] Bike multi-tool (carry on rides)
- [ ] Spare tubes (2 per bike)
- [ ] Tire levers
- [ ] Mini pump or CO2 inflator + cartridges
- [ ] Patch kit
- [ ] Chain lube
- [ ] Bike lock (for campsite/town)

### Truck Bike Rack

- [ ] **Truck bed bike rack** or **tailgate pad**
  - **1Up USA Heavy Duty** (best, $$$$) — bomb-proof, easy load
  - **RockyMounts BackStage** (~$350) — swing-away, bed-mounted
  - **Tailgate Pad** (~$150-300) — cheap, works, bikes touch each other
  - **Thule Insta-Gater** (~$500) — premium tailgate solution

## Camping Gear (Fill Gaps)

### Need to Buy/Verify You Have

- [ ] **Sleeping bags rated for 30°F** (desert nights can drop to freezing)
- [ ] **Sleeping pads** (insulated for cold ground)
- [ ] **Tent large enough for 2 people + gear**
- [ ] **Camp stove + fuel** (MSR, Jetboil, or Coleman dual-fuel)
- [ ] **Cooler** (Yeti/RTIC if you want ice to last, Coleman works)
- [ ] **Water containers** (5-gallon jugs, collapsible, etc.) — need 10+ gallons total
- [ ] **Headlamps** (one per person + spare batteries)
- [ ] **Camp chairs** (if you don't have)
- [ ] **Portable toilet or WAG bags** (for dispersed camping)

## Vehicle Prep

### F150 Maintenance (Do Before Trip)

- [ ] **NEW TIRES - DEADLINE: March 15** (3 weeks before departure)
- [ ] Oil change (if due)
- [ ] Check tire pressure + tread on new tires (including spare)
- [ ] Inspect brake pads
- [ ] Top off all fluids (coolant, brake, washer, etc.)
- [ ] Test 4WD engagement (engage before trip to verify it works)
- [ ] Check battery health
- [ ] Replace windshield wipers if needed
- [ ] Wash/vacuum (start clean)

### Vehicle Gear to Buy/Check

- [ ] **Air compressor** (ARB, Viair, or cheaper option) — for airing down/up tires
- [ ] **Tire pressure gauge**
- [ ] **Recovery boards** (MaxTrax, ActionTrax) — optional but useful for sand/mud
- [ ] **Recovery strap + shackles** (if you don't have)
- [ ] **Spare fuel can** (5-10 gal, check permit rules for White Rim)
- [ ] **Fire extinguisher** (ABC rated, mount in cab)
- [ ] **First aid kit** (vehicle + camping)
- [ ] **Jumper cables or jump starter pack**
- [ ] **Road flares or reflective triangles**
- [ ] **Shovel** (collapsible or full-size)

## Food & Kitchen

### To Buy Before Trip

- [ ] Non-perishable meals (freeze-dried, canned, pasta, rice, etc.)
- [ ] Snacks (bars, nuts, jerky, fruit)
- [ ] Coffee/tea
- [ ] Spices/condiments in small containers
- [ ] Cooking oil
- [ ] Dish soap (biodegradable)
- [ ] Trash bags (heavy-duty)
- [ ] Ziplock bags (various sizes)
- [ ] Aluminum foil
- [ ] Paper towels

### Restock Along the Way

- [ ] Fresh food (buy in towns as you go)
- [ ] Ice (refill in Moab, etc.)
- [ ] Drinking water (top off at towns)

## Personal Gear

- [ ] **Sunscreen SPF 50+** (desert sun is brutal)
- [ ] **Lip balm with SPF**
- [ ] **Sunglasses** (good ones, you'll be squinting a lot)
- [ ] **Hiking boots** (broken in)
- [ ] **Sandals/camp shoes**
- [ ] **Layered clothing** (mornings cold, midday hot)
  - Base layers
  - Fleece/puffy jacket
  - Rain jacket
  - Shorts + pants
  - Long-sleeve sun shirts
- [ ] **Hat** (wide brim or baseball cap)
- [ ] **Buff or bandana** (dust protection)
- [ ] **Work gloves** (for camp setup, firewood)

## Navigation & Comms

- [ ] **Satellite communicator** (Garmin InReach, SPOT) — HIGHLY RECOMMENDED for White Rim
- [ ] **Printed maps** (Utah road atlas, White Rim map)
- [ ] **Offline maps downloaded** (Google Maps, Gaia GPS)
- [ ] **Portable battery pack** (Anker, Goal Zero) — 20,000+ mAh
- [ ] **12V phone charger**
- [ ] **GoPro or action camera** (optional, for riding/scenery)

## 🚁 Drone

**Options:**
- **DJI Mini 4 Pro** (~$759) — Best value, <249g (no FAA registration), 34 min flight time, 4K/60fps
- **DJI Air 3** (~$1,099) — Dual cameras, 46 min flight time, obstacle avoidance
- **DJI Mavic 3** (~$2,199) — Pro-level, Hasselblad camera, 43 min flight time

**Accessories:**
- [ ] Extra batteries (2-3 additional)
- [ ] ND filters for desert sun
- [ ] Hard case for transport
- [ ] Car charger
- [ ] MicroSD cards (128GB+ high-speed)

**Note:** Check NPS regulations — drones prohibited in National Parks (Canyonlands, Arches, Zion, Death Valley, Capitol Reef). OK on BLM land and most dispersed camping areas.

## Moab-Specific Gear

- [ ] **Dust masks or buffs** (Moab = dusty, especially on bikes)
- [ ] **Eye drops** (dry air, dust)
- [ ] **Extra bike tubes** (Moab trails eat tires)
- [ ] **Tubeless sealant** (if running tubeless, bring extra)

## Shopping Timeline

**6-8 weeks before (late Feb/early Mar):**
- Order bikes (can take time to arrive/assemble)
- Buy big-ticket items (rack, sleeping bags, compressor)

**MARCH 8 DEADLINE - Recovery Gear & Camping Equipment:**
- [ ] Order air compressor
- [ ] Order recovery straps/boards
- [ ] Order sleeping bags/pads
- [ ] Order tent (if needed)
- [ ] Order camping stove
- [ ] Order headlamps/lanterns
- [ ] Order water containers
- [ ] Order bike accessories (helmets, packs, tools)
- [ ] Order drone + accessories
- Allow 3+ weeks for shipping, returns, replacements if needed

**MARCH 15 DEADLINE - Vehicle:**
- [ ] **NEW TIRES installed**
- [ ] Oil change
- [ ] Brake inspection
- [ ] All fluids topped off
- [ ] 4WD test
- 3 weeks before departure = plenty of wiggle room

**MARCH 21 DEADLINE - Food Shopping:**
- [ ] Non-perishable meals (freeze-dried, canned, etc.)
- [ ] Snacks (bars, nuts, jerky)
- [ ] Coffee/tea
- [ ] Spices/condiments
- [ ] Cooking oil, dish soap
- [ ] Paper towels, trash bags
- 2 weeks before departure, time to organize/restock if needed

**APRIL 3 (Day before departure):**
- [ ] Fresh food, ice
- [ ] Top off fuel, water
- [ ] Final pack check

## Budget Estimate (Rough)

| Item | Cost |
|------|------|
| 2x Mountain Bikes (mid-range) | $2,000 - $3,000 |
| Bike accessories (helmets, packs, etc.) | $400 - $600 |
| Bike rack/tailgate pad | $150 - $500 |
| Camping gear (if buying new) | $500 - $1,000 |
| Vehicle gear (compressor, straps, etc.) | $200 - $400 |
| Drone + accessories | $900 - $2,500 |
| Food for 14 days | $400 - $600 |
| Fuel (estimate ~2,500 miles @ 15 mpg) | $500 - $700 |
| Campsite fees (non-White Rim) | $100 - $200 |
| **Total** | **$5,150 - $9,500** |

Add buffer for misc/unexpected.
